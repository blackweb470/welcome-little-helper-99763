      ...
      {!isMinimized ? (
        <Card className="w-full h-full shadow-2xl flex flex-col overflow-hidden flex-1">
          ...
          <div className="border-b shrink-0">
            <div className="flex">
              {[
                ...(qaPairs.length > 0 ? [{ id: "faq" as WidgetTab, icon: HelpCircle, label: "FAQ" }] : []),
          ...
          <CardContent className="p-0 flex-1 flex flex-col overflow-hidden min-h-0">
            {activeTab === "faq" && qaPairs.length > 0 && renderFaqContent()}
            {(activeTab === "chat" || qaPairs.length === 0) && renderChatContent()}
          </CardContent>
        </Card>
      ) : (
